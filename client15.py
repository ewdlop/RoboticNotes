...
class RobotController:
    ...
    def evaluate_locomotion(self, params, duration=10.0, threshold=0.2):
        """
        新增「逐步累積距離變化」的 reward：
        - 先計算初始距離 initial_dist = 距立方體距離。
        - 每個 time‐step 都計算 curr_dist，並用 delta = prev_dist - curr_dist 作為 reward 增量（若往立方體方向移動，delta>0，累積正收益；若遠離則 delta<0，扣分）。
        - 最後 fitness = total_delta / duration（平均每秒縮短距離）。
        - 若達到 threshold 以內，額外將 total_delta 增加一大比例獎勵，鼓勵盡快接近。
        """
        # 重置 Robot 底座位置與姿態
        p.resetBasePositionAndOrientation(self.robotId, [0, 0, 1], [0, 0, 0, 1])
        # 先讓機器人穩定一段時間
        ...
        dx0 = robot_pos[0] - cube_pos[0]
        dy0 = robot_pos[1] - cube_pos[1]
        prev_dist = math.sqrt(dx0*dx0 + dy0*dy0)
        total_delta = 0.0
        ...
        for step in range(max_steps):
            ...
            # 若在 step 內距離 <= threshold，就加大獎勵並跳出
            if curr_dist <= threshold:
                reached = True
                total_delta += 5.0  # 額外 bonus，鼓勵快速接近
                break

        # 計算平均每秒縮短距離（若 reached，已含額外獎勵）
        fitness = total_delta / duration
        return fitness

    def hill_climber_optimization(self, generations=50, threshold=0.2):
        """
        Hill-climbing 優化 locomotion 參數（追逐紅色立方體）：
        - 每代產生 candidate_params，再以新的 evaluate_locomotion() 計算 fitness。
        - 適應度越高，表示「平均每秒縮短距離」越大，而且若在訓練時間內追到立方體則有額外 bonus。
        """
