...
class RobotController:
    ...
    def initialize_random_params(self):
        ...
        for joint_idx in self.controllable_joints:
            ...
            offset = random.uniform(lower_limit + 0.2 * range_size, 
                                  upper_limit - 0.2 * range_size)
            amplitude = random.uniform(0.2 * range_size, 0.6 * range_size)  # Increased amplitude
            phase = random.uniform(0, 2 * math.pi)
            frequency = random.uniform(0.8, 2.5)  # Increased frequency range
    ...
    def evaluate_locomotion(self, params, duration=8.0):
        ...
        for step in range(steps):
            ...
            for joint_idx in self.controllable_joints:
                ...
                # Stronger directional bias to help robot move toward cube
                if distance_to_cube > 0.3:  # Only when not very close
                    if 'hip' in joint_name:
                        # Hip joints control forward/backward movement
                        if 'front' in joint_name or '0' in joint_name or '1' in joint_name:
                            bias = 0.2 * direction_x * min(distance_to_cube / 3.0, 1.0)
                        elif 'back' in joint_name or '2' in joint_name or '3' in joint_name:
                            bias = -0.2 * direction_x * min(distance_to_cube / 3.0, 1.0)
                    elif 'ankle' in joint_name:
                        # Ankle joints can help with turning
                        if 'front' in joint_name or '0' in joint_name or '1' in joint_name:
                            bias = 0.1 * direction_y * min(distance_to_cube / 3.0, 1.0)
                        elif 'back' in joint_name or '2' in joint_name or '3' in joint_name:
                            bias = -0.1 * direction_y * min(distance_to_cube / 3.0, 1.0)
                ...
                # Apply control with increased force and velocity
                p.setJointMotorControl2(
                    bodyUniqueId=self.robotId,
                    jointIndex=joint_idx,
                    controlMode=p.POSITION_CONTROL,
                    targetPosition=target,
                    force=300,  # Increased force for better movement
                    maxVelocity=4.0,  # Increased max velocity
                    positionGain=0.1,  # Add position gain for better control
                    velocityGain=0.1   # Add velocity gain for smoother movement
                )
        ...
        if len(robot_positions) >= 2:
            ...
            # Also calculate total movement from start to end
            start_pos = robot_positions[0]
            end_pos = robot_positions[-1]
        ...
        # 1. Speed component (primary reward)
        speed_reward = avg_speed * 2.0
        
        # 2. Distance component (getting closer to cube)
        max_distance = 10.0  # Maximum expected distance
        distance_reward = max(0, max_distance - final_distance) * 0.5
        
        # 3. Movement reward (prevent getting stuck)
        if len(robot_positions) >= 2:
            start_pos = robot_positions[0]
            end_pos = robot_positions[-1]
            total_movement = math.sqrt(
            ...
            movement_reward = min(total_movement, 5.0) * 0.3
        else:
            movement_reward = 0
        
        # 4. Bonus for getting very close to cube
        proximity_bonus = 0
        if final_distance < 1.0:
            proximity_bonus = (1.0 - final_distance) * 2.0
        
        fitness = speed_reward + distance_reward + movement_reward + proximity_bonus
        
        print(f"  Speed: {avg_speed:.3f}, Distance: {final_distance:.3f}, Movement: {total_movement:.3f}, Fitness: {fitness:.3f}")
    ...
    def hill_climber_optimization(self, generations=50):
        ...
        for generation in range(generations):
            ...
            # Hill climbing: accept if better, or occasionally accept worse solutions to escape local minima
            accept_candidate = False
            if candidate_fitness > current_fitness:
                accept_candidate = True
                improvement_msg = "IMPROVED"
            elif generation < generations // 4:  # Early exploration
                # Accept worse solutions with small probability early on
                probability = 0.1 * (1 - generation / (generations // 4))
                if random.random() < probability:
                    accept_candidate = True
                    improvement_msg = "EXPLORATION"
                else:
                    improvement_msg = "no improvement"
            else:
                improvement_msg = "no improvement"
            ...
            if accept_candidate:
                ...
                print(f"Generation {generation+1}: {improvement_msg} fitness = {current_fitness:.4f}")
                
                if current_fitness > self.best_fitness:
                    self.best_fitness = current_fitness
                    self.best_params = current_params.copy()
            else:
                print(f"Generation {generation+1}: fitness = {candidate_fitness:.4f} ({improvement_msg})")
